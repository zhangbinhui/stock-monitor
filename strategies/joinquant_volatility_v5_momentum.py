"""
波动率突破策略 v5M (Momentum) - 聚宽研究环境
=============================================

【和 v5 的唯一区别：股票池】
  v5  → 深跌股（从高点大幅回撤的股票）
  v5M → 强势股（均线多头排列 + 近期上涨趋势）

  其余完全一样：触发逻辑、卖出策略、资金模拟、参数空间。
  目的是对比：同样的波动率突破信号，在不同股票池上效果差多少。

【强势股选股条件】
  - 市值≥50亿（流动性）
  - 均线多头排列：收盘价 > MA20 > MA60（短期趋势向上）
  - 近20日涨幅 > 0（确认上涨中）
  - 排除ST、上市不满2年

【策略逻辑】
  强势股出现波动率突破 = 主力加仓加速，动量延续概率高
  （对比深跌股的异常拉升 = 更可能是死猫跳/诱多）
...
print("✅ Cell 1 配置完成")
print("=" * 60)
print(f"  📌 股票池: 市值≥{MIN_MARKET_CAP/1e8:.0f}亿，均线多头(MA{MA_SHORT}>MA{MA_LONG}) + 近{MOMENTUM_DAYS}日上涨")
...
# ============================================================
# Cell 2：构建滚动股票池（★ 强势股版本）
# ============================================================
#
# 【和深跌股版本的区别】
#   深跌版：当前价 < 1年高点 × 回撤比例 → 入池（越跌越入池）
#   强势版：收盘价 > MA20 > MA60 且近20日涨幅>0 → 入池（越涨越入池）
#
# 【为什么要滚动】
#   同一只股票不是一直强势的。比如贵州茅台2024上半年跌，下半年涨，
#   只有涨的那段时间才应该被监控。

def build_rolling_pool(end_date, backtest_years=BACKTEST_YEARS,
                       min_cap=MIN_MARKET_CAP):
    """
    构建强势股滚动股票池：每个交易日判断哪些股票处于「多头排列+上涨趋势」
    
    入池条件（每天独立判断）：
      1. 收盘价 > MA20（站在短期均线上方）
      2. MA20 > MA60（短期均线在长期均线上方 = 多头排列）
      3. 近20日涨幅 > 0（确认上涨中，不是横盘）
    ...
    """
    print(f"\n{'='*60}")
    print(f"📊 第一步：构建强势股滚动股票池")
    print(f"{'='*60}")
    print(f"  入池条件: 收盘价>MA{MA_SHORT} 且 MA{MA_SHORT}>MA{MA_LONG} 且 近{MOMENTUM_DAYS}日涨幅>0")

    # 时间范围：回测区间 + 往前多留1年（算MA60需要至少60天数据）
    bt_start = pd.to_datetime(end_date) - timedelta(days=365 * backtest_years)
    data_start = bt_start - timedelta(days=365)  # 多留1年给均线计算
    ...
    for i in range(0, len(candidate_codes), 50):
        ...
        prices = get_price(batch, start_date=data_start_str, end_date=end_date,
                           frequency='daily', fields=['close'], panel=True)
        for code in batch:
            try:
                if isinstance(prices['close'], pd.DataFrame):
                    closes = prices['close'][code].dropna()
                else:
                    continue
                if len(closes) < MA_LONG + MOMENTUM_DAYS + 10:
                    continue
                ...
                valid_dates = set()
                for j in range(MA_LONG, len(close_arr)):
                    ...
                    # MA20 = 过去20天收盘价的平均
                    ma_short = close_arr[j-MA_SHORT+1:j+1].mean()
                    # MA60 = 过去60天收盘价的平均
                    ma_long = close_arr[j-MA_LONG+1:j+1].mean()
                    
                    # 条件1: 收盘价 > MA20（站在短期均线上方）
                    # 条件2: MA20 > MA60（多头排列）
                    # 条件3: 近20日涨幅 > 0
                    if j >= MOMENTUM_DAYS:
                        past_price = close_arr[j - MOMENTUM_DAYS]
                        momentum = (current - past_price) / past_price
                    else:
                        momentum = 0
                    
                    if current > ma_short and ma_short > ma_long and momentum > 0:
                        valid_dates.add(d)
    ...
    print(f"  （入池天数 = 该股票处于多头排列的总天数，越多说明趋势越持久）")
